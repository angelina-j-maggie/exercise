...
When conducitng exploratory analysis, it was shown that many of the variables were not well populated and so only the sixty in the subset *training_sub* and *testing_sub* were used. 
...
Four different machine learning techniques were compared to determine which was the most accurate, using the *my_training* and *my_testing* datasets. The most accurate was then used to predict the *classe* in the given testing set. The first is linear discriminant analysis (LDA) which provides a linear combination of variable effects, much like generalised linear regression, to explain the prediction varialbe. Random forest (RF) is an ensemble learning technique for classification (in this case for predicting *classe*), by constructing multiple decision trees to predict the classifications. The third technique is support vector machine (SVM) learning, which is a supervised learning technique meaning that it assigns new examples based on previously learnt classifications. NOtably it isn't probabilistic. The final technique is generalised boosting methods (GBM), which is a probabilistic method that produces a prediction model in a stepwise method.
...
After computation, the 4 methods were compared below to determine their accuracy. The confusion matrices show that the most accurate technique was random forest.
...
In addition, the recievor operated curve (ROC) was computed for the random forest model below. This gives a cross-validation that the random forest has both good specificity and sensitivity. The plots for variables A, B, C of *classe* give an AUC of 1 (not shown), with D and E still giving high values, implying adequate sensitivity.
...
ctrl <- trainControl(method="cv", 
                     ...
                     classProbs=T,
                     savePredictions = T)
rfFit <- train(classe ~ ., data=training_sub, 
...
plot.roc(rfFit$pred$obs[selectedIndices],
         rfFit$pred$D[selectedIndices])

plot.roc(rfFit$pred$obs[selectedIndices],
         rfFit$pred$E[selectedIndices])
...
The most accurate technique was random forest, which gave the following results in the provided testing set. It had an accuracy of 99%, whereas the other techniques were: 70% for LDA, 96% for GBM and 92% for SVM. 
